...
app.post('/gerar-pdf', (req, res) => {
  ...
  doc.fontSize(14).font('Helvetica').text('oportunidade que se envolveu em acidente de trânsito que não resultou vítima, no entanto, foi autuado pelo suposto cometimento da infração prevista no Art. 176 do Código de Trânsito Brasileiro.', { align: 'left' });
  doc.moveDown();
  ...
  doc.moveDown(); 
  doc.fontSize(14).font('Helvetica').text('II - se, no prazo máximo de trinta dias, não for expedida a notificação da autuação.', { align: 'right' });
  doc.moveDown(); 
...
doc.moveDown();
  ...
  doc.fontSize(14).font('Helvetica').text('A titulo de exemplo cita-se o art. 176, I do CTB, no entanto, estendendo-se aos demais incisos do mesmo artigo, que com base no MBFT define  quanto ao procedimento a ser adotado prevê a necessidade de arrolar/mencionar especificamente a vítima, bem como a natureza das lesões sofridas.', { align: 'left' });
  doc.moveDown();
